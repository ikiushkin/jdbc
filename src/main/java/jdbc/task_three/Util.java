...
public class Util {
                ...
                for (int j = 0; j < 30000; j++) {
                    ...
                    if (id1 != id2) {
                        try (Connection conn = getConnection(jdbcPropertyFile);
                            ...
                            ps.setLong(1, id1);
                            ps.setLong(2, id2);
                            ps.setDate(3, createData(new Date(125, 2, 1), new Date(125, 6, 30)));
                            ps.executeUpdate();
    ...
    public static List<User> getUsersWithFriendsAndLikes(int countForFriends, int countForLikes) {
        String SQL_SELECT = String.format("select * from users\n" +
                "where\n" +
                "    ID in (\n" +
                ...
                "        where DATE(CREATED_DATE) between '2025-03-01' and '2025-03-31'\n" +
                "        group by USER_ID1\n" +
                "        having count(USER_ID2) > %s\n" +
                "    )\n" +
                "order by ID;", countForLikes, countForFriends);
